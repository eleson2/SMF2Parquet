...
- [ ] 🔁 Per-type synthetic record + value assertions (needs richer fixtures — Phase 9).

Note: stub types (71/72/73/76/77/78/79/113) currently emit header-only tables; they gain
type-specific columns when their parse cores are fleshed out (Phase 7).

## Phase 7 — Field-offset verification (correctness) ❗

- [ ] 🔁 Per type, resolve every `TODO: verify offsets` against IBM SMF Explorer mappings
      (https://ibm.github.io/IBM-SMF-Explorer/mappings/) and `SMF-parser/reference_doc/`.
- [ ] 🔁 Validate against a real captured record where available; record evidence in
      `docs/validation/smfNN.md`.
- [ ] Add `static_assert` tests for any new/changed conversions (MF-records-to-C style).

## Phase 8 — CLI, config, observability

- [ ] CLI: `smf2parquet <input.smf> <out_root> [--run-id ID] [--batch-ts TS]
      [--compression zstd|snappy] [--types 30,70,...]`.
- [ ] Optional config file for defaults (out_root, compression, type filter).
- [ ] Exit-time stats report: per-type record + row counts, raw/unknown/malformed counts,
      bytes, wall time. Exit codes: 0 ok, 1 usage, 2 I/O/write error.
- [ ] Structured log line per run (JSON) for the batch scheduler to consume.

## Phase 9 — Testing & CI

- [x] Self-contained `tests/gen_test_smf.cpp` (no deps; target `gen_test_smf`): all 13 types
      + unknown 250 across two dates. Verified: 16 records → all 13 tables + `raw`, with smf30
      & raw spanning two `smf_date=` partitions (covers the midnight-straddle edge case).
