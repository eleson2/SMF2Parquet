...
struct Smf70_1Builders {
    ...
    explicit Smf70_1Builders(arrow::MemoryPool* pool)
        : common(pool),
...
class Smf70_1ParquetSink {
public:
    Smf70_1ParquetSink(std::string customer, std::string out_root, std::string run_id,
                     std::string source_file, int64_t ingest_us)
        : source_(std::move(source_file)), ingest_(ingest_us),
          table_("smf70-1", std::move(customer), Smf70_1Builders::schema(), std::move(out_root), std::move(run_id)) {}
...
private:
    std::string source_;
    int64_t     ingest_;
    PartitionedTable<Smf70_1Builders> table_;
};
