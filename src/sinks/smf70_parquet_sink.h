...
public:
    Smf70ParquetSink(std::string customer, std::string out_root, std::string run_id,
                     std::string source_file, int64_t ingest_us)
        : source_(std::move(source_file)), ingest_(ingest_us),
          table_("smf70", std::move(customer), Smf70Builders::schema(), std::move(out_root), std::move(run_id)) {}
