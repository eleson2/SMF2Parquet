...
struct Smf72_3Builders {
    ...
    uint64_t append(const smf::Smf72Record& r, std::string_view src, int64_t ingest_us) {
        ...
        for (const auto& p : r.periods) {
            ...
            arrow_ok(policy_name  ->Append(r.control.policy_name));
            arrow_ok(workload_name->Append(r.control.workload_name));
            arrow_ok(class_name   ->Append(r.control.class_name));
