...
struct Smf74_2MemberBuilders {
    ...
    explicit Smf74_2MemberBuilders(arrow::MemoryPool* pool)
        : common(pool),
    ...
    uint64_t append(const smf::Smf74_2Record& r, std::string_view src, int64_t ingest_us) {
        const uint64_t n = r.members.size();
        if (n == 0) return 0;
    ...
    arrow::ArrayVector finish() {
        ...
        c.push_back(fin(group_name.get()));
        c.push_back(fin(member_name.get()));
...
public:
    Smf74_2_MemberParquetSink(std::string customer, std::string out_root, std::string run_id,
        ...
        : source_(std::move(source_file)), ingest_(ingest_us),
          table_("smf74-2m", std::move(customer), Smf74_2MemberBuilders::schema(), std::move(out_root), std::move(run_id)) {}
...
private:
    std::string source_;
    int64_t     ingest_;
    PartitionedTable<Smf74_2MemberBuilders> table_;
};
