...
struct Smf74_2_PathBuilders {
    ...
    explicit Smf74_2_PathBuilders(arrow::MemoryPool* pool)
        : common(pool),
    ...
    arrow::ArrayVector finish() {
        ...
        c.push_back(fin(system_name.get()));      c.push_back(fin(device_num.get()));
        c.push_back(fin(direction.get()));        c.push_back(fin(signals_sent.get()));
...
public:
    Smf74_2_PathParquetSink(std::string customer, std::string out_root, std::string run_id,
        ...
        : source_(std::move(source_file)), ingest_(ingest_us),
          table_("smf74-2-path", std::move(customer), Smf74_2_PathBuilders::schema(), std::move(out_root), std::move(run_id)) {}
...
private:
    std::string source_;
    int64_t     ingest_;
    PartitionedTable<Smf74_2_PathBuilders> table_;
};
