...
struct Smf75_1Builders {
    ...
    explicit Smf75_1Builders(arrow::MemoryPool* pool)
        : common(pool),
    ...
    uint64_t append(const smf::Smf75Record& r, std::string_view src, int64_t ingest_us) {
        const uint64_t n = r.pagesets.size();
        if (n == 0) return 0;
        ...
        const auto sc = r.product.sample_count;
        ...
        for (const auto& ps : r.pagesets) {
            ...
            arrow_ok(sample_count     ->Append(sc));
...
class Smf75_1ParquetSink {
public:
    Smf75_1ParquetSink(std::string customer, std::string out_root, std::string run_id,
                     std::string source_file, int64_t ingest_us)
        : source_(std::move(source_file)), ingest_(ingest_us),
          table_("smf75-1", std::move(customer), Smf75_1Builders::schema(), std::move(out_root), std::move(run_id)) {}
...
private:
    std::string source_;
    int64_t     ingest_;
    PartitionedTable<Smf75_1Builders> table_;
};
